...
public:
  ...
  // ---- Offband-only prefs -------------------------------------------------
  // Appended after upstream's members. These have no upstream counterpart, so
  // they must be carried across every base update -- and they must appear in
  // structure() below or they will not survive the /prefs.json migration.
  // #542: status/traffic LED enable (1 = normal, 0 = suppressed). Default 1 (ON).
  // Only meaningful where MainBoard::canControlLed() is true.
  uint8_t ui_led_enabled = 1;
  // #542 A2: OLED mode. 0 = auto (on, blanks after timeout -- default/today),
  // 1 = always on (no blank), 2 = always off (dark). Only meaningful on builds
  // with a DISPLAY_CLASS.
  uint8_t ui_display_mode = 0;
  // #562: caplog (MeshLog capture) persistence -- common across all CommonCLI
  // roles (repeater / observer / room-server / sensor).
  uint8_t caplog_enabled = 0; // 0 = off (default), 1 = on
  uint8_t caplog_level = 0;   // MLOG_* level; capture lines with level <= this
...
private:
  ...
  protected:
    void structure() override {
      ...
      // Upstream 1.17.0 ships this mapped to rx_boosted_gain -- a copy/paste bug
      // that leaves radio_fem_rxgain unpersisted and makes `fem_rxgain` clobber
      // `rxgain`. Fixed upstream in PR #3137; applied here so we converge.
  ...
  // Offband-only groups. Same shape as upstream's sub-objects above so the
  // nesting convention stays uniform (#197) and future rebases stay cheap.
  class UIPrefs : public ConfigSerializer {
    NodePrefs* _parent;
  protected:
    void structure() override {
      def("led", _parent->ui_led_enabled);      // #542
      def("disp", _parent->ui_display_mode);    // #542 A2
    }
  public:
  ...
  protected:
    void structure() override {
      def("caplog_en", _parent->caplog_enabled);   // #562
...
protected:
  void structure() override {
    ...
    def("room", room);
    def("power", power);
    def("ui", ui);      // Offband #542
    def("log", log);    // Offband #562/#566
  }
...
class CommonCLI {
  ...
  // #627: stock-MeshCore variant of the legacy reader (differs only at 290-294),
  // and the evidence gatherer that decides which of the two to use.
  void loadPrefsUpstreamInt(FILESYSTEM* _fs, const char* filename);
  offband::PrefsLayoutEvidence gatherLegacyEvidence(FILESYSTEM* _fs, const char* filename);
