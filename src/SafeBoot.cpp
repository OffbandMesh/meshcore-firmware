 ...
 *
 * Ported from Meshtastic PR #10391 (commit 4c7e1ee8) into MeshCore.
 * Adaptations from upstream:
 *   - ARCH_ESP32 -> ESP32, ARCH_NRF52 -> NRF52_PLATFORM (MeshCore conventions)
 *   - BATTERY_PIN -> PIN_VBAT_READ, ADC_CTRL -> PIN_ADC_CTRL,
 *     BUTTON_PIN -> PIN_USER_BTN (MeshCore conventions)
 *   - Dropped #include "configuration.h" (MeshCore has no such header)
 *   - Dropped USERPREFS_POWER_SAFE_BOOT_* overrides (MeshCore has no
 *     userPrefs.jsonc)
 *   - nRF52 GPREGRET2 coexistence (Path A): SafeBoot reserves bit 7 as
 *     ownership marker; lower 7 bits pack (stage:3 + attempts:3 + unclean:1).
 *     Coexists with MeshCore's NRF52Board.cpp SHUTDOWN_REASON_* codes
 *     (all of which have bit 7 = 0). Cleanup tracked as #96 F12 / #144.
 */
...
// ---------------------------------------------------------------------------
// Defaults (chemistry: LiIon single cell). Per-board overrides come from
// variant platformio.ini build flags (DEFAULT_SAFE_BOOT_*).
//
...
// the next attempt, even if Vbat now reads OK. Anti boot-loop safeguard.
...
// Magic stored in retention memory to validate the persisted state (ESP32).
...
// MeshCore's NRF52Board.cpp also writes to GPREGRET2 (SHUTDOWN_REASON_*
// codes 0x42 / 0x4C / 0x55, all with bit 7 = 0). To coexist, SafeBoot
// reserves BIT 7 as an ownership marker:
//
//   bit  [7]   : "SafeBoot owns this byte" marker (must be 1)
//   bits [3..5]: attempts counter (0..7, saturating)
//   bit  [6]   : "previous reset unclean" sticky flag
//   bits [0..2]: backoff stage (0..7) -> sleep = base * 2^stage, capped
//
// If bit 7 is 0, the byte was written by NRF52Board (or is fresh
// power-on with RESETREAS == 0). SafeBoot treats it as cold-boot.
//
// Note this narrows the attempts counter from upstream's 4 bits (max 15)
// to 3 bits (max 7). Functionally equivalent: attempts >= 4 already
// produces sleep_secs = MAX_RECHECK_SECS (cap = 600s) on default
// thresholds, so attempts 4..7 and 8..15 encode the same backoff.
//
// F12 / #144 will reclaim bit 7 once MeshCore's dormant nRF52
// power-mgmt is reconciled, restoring the upstream encoding.
constexpr uint8_t SAFEBOOT_OWN_BIT   = 0x80;
constexpr uint8_t MASK_STAGE         = 0x07;
constexpr uint8_t SHIFT_ATTEMPTS     = 3;
constexpr uint8_t MASK_ATTEMPTS      = 0x07 << SHIFT_ATTEMPTS; // bits 3..5
constexpr uint8_t FLAG_UNCLEAN       = 0x40;                   // bit 6
constexpr uint8_t SAFEBOOT_ATTEMPTS_MAX = 7;
...
PersistedState loadPersisted()
{
    ...
    // If the ownership marker is clear, the byte belongs to NRF52Board
    // (a SHUTDOWN_REASON_* code) or is fresh power-on. Return a fresh
    // state so SafeBoot treats this as a cold-boot.
    if ((reg & SAFEBOOT_OWN_BIT) == 0) {
        s.magic = SAFE_BOOT_MAGIC;
        s.backoff_secs = DEFAULT_SAFE_BOOT_RECHECK_SECS;
        s.attempts = 0;
        s.flags = 0;
        s.last_vbat_mv = 0;
        return s;
...
void storePersisted(const PersistedState &s)
{
    ...
    uint8_t attempts = (s.attempts > SAFEBOOT_ATTEMPTS_MAX) ? SAFEBOOT_ATTEMPTS_MAX : (uint8_t)s.attempts;
    uint8_t reg = SAFEBOOT_OWN_BIT
                  | (uint8_t)(stage & MASK_STAGE)
                  | (uint8_t)((attempts << SHIFT_ATTEMPTS) & MASK_ATTEMPTS)
                  | (uint8_t)((s.flags & 1u) ? FLAG_UNCLEAN : 0);
#if __has_include(<nrf_soc.h>)
...
// Instead we read PIN_VBAT_READ directly using the board-specific
...
{
#if defined(ESP32)
    // We're called BEFORE board.begin(), so the chip is in its post-reset
    // state. Check both the modern API and (implicitly) the legacy one to
    // catch C3 / S3 / C6 too.
...
// ---------------------------------------------------------------------------
// Deep sleep entry (used while in SafeBoot, before normal board init).
// We deliberately bypass any MeshCore sleep helper here because they touch
// NodeDB, screen, BLE and others that we have not initialised yet.
// ---------------------------------------------------------------------------
[[noreturn]] void enterSafeBootSleep(uint32_t secs)
{
    ...
    // up downstream in normal sleep helpers when buttons are configured;
    // we do the minimum here so we don't depend on it.
#if SOC_PM_SUPPORT_EXT_WAKEUP
    uint64_t mask = ((uint64_t)1) << PIN_USER_BTN;
    ...
    //      Vbat rise. Optimal path: ~1-3 uA.
    //   2) PIN_USER_BTN defined          -> SYSTEM_OFF + GPIO SENSE on the
...
#if defined(PIN_USER_BTN) && __has_include(<nrf_gpio.h>)
    ...
    nrf_gpio_cfg_sense_set(PIN_USER_BTN, NRF_GPIO_PIN_SENSE_LOW);
#endif

#if defined(BATTERY_LPCOMP_INPUT) || (defined(PIN_USER_BTN) && __has_include(<nrf_gpio.h>))
    ...
    // back to the bare register write.
    Serial.flush();
#if __has_include(<nrf_soc.h>)
    ...
    // Honest tradeoff: current ~1.5-3 mA instead of ~3 uA. The boot loop
