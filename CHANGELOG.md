...
### Added
- **Queen City Con 0x4 badge: Offband on a keyboard badge** (feature [#1172](https://github.com/OffbandMesh/meshcore-firmware/issues/1172); bring-up epic [#1173](https://github.com/OffbandMesh/meshcore-firmware/issues/1173), interface epic [#1188](https://github.com/OffbandMesh/meshcore-firmware/issues/1188)). A ProMicro nRF52840 with an Ra-01SH radio, a 128x64 OLED, a CardKB keyboard and one button. Bring-up landed the board class and pin map with `Serial1` kept off the buzzer and LED pins ([#1177](https://github.com/OffbandMesh/meshcore-firmware/issues/1177), [#1180](https://github.com/OffbandMesh/meshcore-firmware/issues/1180)), a ThingPulse-font renderer for any `DisplayDriver` ([#1179](https://github.com/OffbandMesh/meshcore-firmware/issues/1179)), the conference splash and its MSB-first generator ([#1181](https://github.com/OffbandMesh/meshcore-firmware/issues/1181), [#1182](https://github.com/OffbandMesh/meshcore-firmware/issues/1182)), badge battery math with a SafeBoot ADC sample-time hook ([#1178](https://github.com/OffbandMesh/meshcore-firmware/issues/1178)), and a diag env with a bring-up self-test legend ([#1183](https://github.com/OffbandMesh/meshcore-firmware/issues/1183)), an ID beacon on the spare pads ([#1210](https://github.com/OffbandMesh/meshcore-firmware/issues/1210)) and SafeBoot's battery reading on screen and through the log mirror ([#1185](https://github.com/OffbandMesh/meshcore-firmware/issues/1185), [#1211](https://github.com/OffbandMesh/meshcore-firmware/issues/1211)).
  - The interface epic adds **Messages as Home**, a thread with inline compose inside the payload budget, channel and DM sends with retries and an ACK tick when a repeat is heard, a message store with conversations and unread counts, and Contacts, Nearby, Status, GPS and Settings on the button cycle ([#1227](https://github.com/OffbandMesh/meshcore-firmware/issues/1227)–[#1235](https://github.com/OffbandMesh/meshcore-firmware/issues/1235)).
  - Badge text starts in the large size, the screen timeout is a setting defaulting to one minute, and Settings → Battery shows the reading, the full point in use and where it came from ([#1244](https://github.com/OffbandMesh/meshcore-firmware/issues/1244), [#1245](https://github.com/OffbandMesh/meshcore-firmware/issues/1245), [#1254](https://github.com/OffbandMesh/meshcore-firmware/issues/1254)).
  - **The badge's diag build ships as a product**, and badge binaries publish with every release ([#1243](https://github.com/OffbandMesh/meshcore-firmware/issues/1243), [#1241](https://github.com/OffbandMesh/meshcore-firmware/issues/1241)) — a tester on unproven hardware can produce a log.
...
- **`wdt hang`** — diagnostic-only (`-D WDT_TEST_HANG`, never in a release image) and console-only: deliberately hangs the main loop so the watchdog can be proven on hardware. `WDT_TEST_HANG_AFTER_MS` does the same unattended, in every role ([#1171](https://github.com/OffbandMesh/meshcore-firmware/issues/1171)).
- **Observer log forwarding to a syslog sink** ([#1059](https://github.com/OffbandMesh/meshcore-firmware/issues/1059), [#1060](https://github.com/OffbandMesh/meshcore-firmware/issues/1060), [#1061](https://github.com/OffbandMesh/meshcore-firmware/issues/1061), [#1193](https://github.com/OffbandMesh/meshcore-firmware/issues/1193), [#1194](https://github.com/OffbandMesh/meshcore-firmware/issues/1194)): the commands are wired on all six observer envs, the sink is stored in NVS so it survives a reboot, the window can run until switched off, tags are sanitized and carry a pubkey tag, arming announces itself, and the capture ring is read through a non-destructive cursor so forwarding never consumes what it forwards. An armed forward also names a capture stop to the sink ([#1240](https://github.com/OffbandMesh/meshcore-firmware/issues/1240)), so a reader knows capture is off while the forward is still armed.
...
- **ESP32 repeaters and companions now arm the runtime watchdog; room servers gain one on every platform** ([#1083](https://github.com/OffbandMesh/meshcore-firmware/issues/1083), epic [#446](https://github.com/OffbandMesh/meshcore-firmware/issues/446)). Since [#266](https://github.com/OffbandMesh/meshcore-firmware/issues/266) the `startWatchdog`/`feedWatchdog` calls in the repeater and companion sat inside `#if defined(NRF52_PLATFORM)`, so no ESP32 repeater or companion ever armed the ESP-IDF task watchdog — a hung node stayed hung until someone reset it by hand. The guard's reason (bridge builds blocking >30 s on network calls) no longer held: MQTT runs in its own task and the loop's network calls time out in seconds. **A hung main loop on any ESP32 board now reboots within 30 s** with `reset_reason=TASK_WDT`; nRF52 behaviour is unchanged. The timeout is one build flag, `WDT_TIMEOUT_SECS` (default 30), for any env that proves it needs longer.
  - CI now blocks the regression: four `config-lint` steps fail if either call is ever placed inside a platform `#if` again ([#1160](https://github.com/OffbandMesh/meshcore-firmware/issues/1160)), and the guard sees through continuations and same-file aliases ([#1281](https://github.com/OffbandMesh/meshcore-firmware/issues/1281)).
  - Design of record: `docs/architecture/2026-09-11-446-esp32-task-watchdog.md`.
- **A stock MeshCore client no longer chokes on our own debug output** ([#1114](https://github.com/OffbandMesh/meshcore-firmware/issues/1114)): on a `_usb` companion, `Serial` *is* the framed companion protocol, and three paths wrote plain text into it. The client parsed one "unexpected frame type" per byte, settings rendered empty and the channel list never arrived.
- **Companion USB frames no longer stall at 64-byte multiples** ([#1093](https://github.com/OffbandMesh/meshcore-firmware/issues/1093)): a frame whose on-wire length is a multiple of the USB max packet size was held host-side until the next write. The terminator is now deferred, which keeps the non-blocking frame path non-blocking. USB-CDC console replies gained the same zero-length terminator ([#1039](https://github.com/OffbandMesh/meshcore-firmware/issues/1039)).
- **`caplog forward` no longer owns the WiFi link** ([#1048](https://github.com/OffbandMesh/meshcore-firmware/issues/1048)): arming overwrote whatever the operator had set with `wifi on`, and disarming tore the link down. Arming, disarming and expiry now only open and close the forward window.
- **The badge's diag build no longer puts the wearer's position on an exposed pad** ([#1247](https://github.com/OffbandMesh/meshcore-firmware/issues/1247)): the periodic GPS line carries `pos=valid` / `pos=invalid` and no coordinates.
- **The badge battery bar tells the truth** ([#1246](https://github.com/OffbandMesh/meshcore-firmware/issues/1246), [#1252](https://github.com/OffbandMesh/meshcore-firmware/issues/1252), [#1254](https://github.com/OffbandMesh/meshcore-firmware/issues/1254)): ends a real cell reaches instead of 3.0–4.2 V, the lithium curve measured from the RC52 [#1004](https://github.com/OffbandMesh/meshcore-firmware/issues/1004) burn instead of a straight line, an average so a transmit stops moving the number, and a 100% point the badge learns from its own charger settling — with a calibrate row and a way back to auto.
- **Any key wakes the badge's display** ([#1239](https://github.com/OffbandMesh/meshcore-firmware/issues/1239)); **first boot starts with the GPS on and the US radio preset** ([#1236](https://github.com/OffbandMesh/meshcore-firmware/issues/1236)); the badge's diag log mirror moved to the one pad that is actually connected, and the wrong P0.15 legend line is gone ([#1185](https://github.com/OffbandMesh/meshcore-firmware/issues/1185)).
...
- **CI builds every shipped RC32 and RCC6 env** ([#877](https://github.com/OffbandMesh/meshcore-firmware/issues/877)): `release-envs.txt` ships 15 envs from those two boards and the build matrix compiled none of them, so a change that broke either board merged green and surfaced only when a release tag was cut. Since [#1016](https://github.com/OffbandMesh/meshcore-firmware/issues/1016) one failed env blocks the whole release.
- **A beta's release notes come from that beta's own CHANGELOG section** ([#1294](https://github.com/OffbandMesh/meshcore-firmware/issues/1294)): the release body was matched after stripping the pre-release suffix, so every beta published the base version's section — beta6's page opened with "### Fixed (beta2)". The tagged version's own section now wins, with the base version kept as the fallback for `-rc` tags.
- **Standards pointers moved to a local file and private data redacted** ([#1098](https://github.com/OffbandMesh/meshcore-firmware/issues/1098)): this repository is public, and tracked files cited a private engineering-standards document by name and rule number while carrying private-subnet addresses and a personal email.

### Known issues shipping in this beta
- **The badge holds its GPS powered the whole time it is enabled** ([#1288](https://github.com/OffbandMesh/meshcore-firmware/issues/1288)): with a GPS module fitted, 2.5–3 days of battery against over a week without one, measured by the badge's designer. Duty cycling is filed and is not in this build.
- This tag is cut from `epic/1188-qcc-keyboard`, not from `firmware-base`: the badge interface epic has not merged, and its verification task ([#1208](https://github.com/OffbandMesh/meshcore-firmware/issues/1208)) is open.
