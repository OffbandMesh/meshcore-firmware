...
- **Diagnostic builds for testers — get a file to someone WITHOUT cutting a release (#957):** dispatch the `diag-build.yml` workflow (`gh workflow run diag-build.yml -R OffbandMesh/meshcore-firmware --ref firmware-base -f env=<env>`); it builds that env with the diag triad (FORCE_CAPLOG + BOOT_BEACON + MESHLOG_UART0) and publishes the `.bin` to the public **`diag-builds`** rolling pre-release → a self-serve tester link `https://github.com/OffbandMesh/meshcore-firmware/releases/download/diag-builds/<env>-diag-<sha>.bin` (no auth, no forwarding). `diag-builds` is NOT a version release. **Full how-to: [`docs/diagnostics/diag-builds.md`](docs/diagnostics/diag-builds.md)** (which `.bin` to flash, reproducing a specific beta, etc.).
