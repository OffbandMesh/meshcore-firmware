...
// The gauge is OPTIONAL. It lives on the Feather rig alongside the RC32; a WROOM
// sniffing a different board may have no gauge wired at all. Set to 0 to compile
// the I2C code out entirely rather than probe for hardware that is not present.
#ifndef SNIFF_GAUGE
  #define SNIFF_GAUGE 0
#endif

// Board-conditional, for the same reason the sniff/RST/BOOT pins are.
//
// A2/A3 are Feather names resolved by its variant header. A CLASSIC ESP32 (and
// most non-Feather boards) does not define them at all, so the generic build
// fails to compile with:
//
//   error: 'A2' was not declared in this scope; did you mean 'T2'?
//
// which reads like a missing library rather than a board mismatch. Raw GPIOs on
// the generic side.
//
// GPIO21/22 are the classic ESP32's conventional I2C pair and are free here --
// Wire is never started by this sketch, so nothing contends for them. They are
// also safe on an S3.
...
static void gaugeTick(uint32_t now) {
  ...
  // VCELL LSB is 78.125 uV. The obvious form -- (vcell * 78125) / 1000000 --
  // OVERFLOWS uint32_t: at vcell=0xF910 (4981 mV) the product is 4,981,250,000
  // against a 4,294,967,295 ceiling, wrapping to a reported 686 mV. Observed on
  // hardware while the pack was on USB.
  //
  // 78125/1000000 reduces exactly to 5/64, so scaling by that instead keeps the
  // product bounded (65535*5 = 327,675) and loses no precision -- it is the same
  // ratio, not an approximation.
  uint32_t mv = ((uint32_t)vcell * 5UL) / 64UL;
