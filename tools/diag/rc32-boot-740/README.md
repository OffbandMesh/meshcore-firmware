...
> diag build: the shipped `heltec_rc32_companion_radio_usb` env plus the shared
> `[rc32_diag]` block (`OFFBAND_BOOT_BEACON`, `OFFBAND_LOG_MIRROR_UART`,
> `OFFBAND_FORCE_CAPLOG`) and `OFFBAND_POWER_TELEMETRY`. It still carries the
> beacon these steps rely on, so the commands below work unchanged. It is **not** the
