...
void UITask::renderCurrScreen() {
  ...
  if (_alert[0]) {
    ...
    // #822: the one shared splash. _version_info is "<mc ver> (<build date>)";
    // the component takes the two apart rather than re-deriving them here.
...
// ESP32 keeps its UI-driven status LED unchanged. The RGB branch below is upstream's
// (1.17.0); the guard rides on its #elif so both features coexist.
#ifdef STATUS_LED_RGB
...
#elif defined(PIN_STATUS_LED) && !defined(NRF52_PLATFORM)
  static int state = 0;
